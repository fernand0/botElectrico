...
def get_data(now):
    ...
    filepath = os.path.join(CACHE_DIR, f"{now.year}-{now.month:02d}-{now.day:02d}_data.json")
...
def get_time_frame(now, weekday):
    ...
    if weekday > 4:
        ...
        for frame_type, hours in TIME_RANGES.items():
            ...
            if start <= now < end:
                return start, end, frame_type, hours, frame_type if not frame_type[-1].isdigit() else frame_type[:-1]
        frame = ["00:00", "24:00"]
        frame_type = "valle"
    return convert_time_to_datetime(frame[0]), convert_time_to_datetime(frame[1]), frame_type, frame, frame_type
...
def find_min_max_prices(data, hours):
    ...
    prices = [float(val['PCB'].replace(',', '.')) / 1000 for val in data["PVPC"][start_hour:end_hour]]
...
def generate_message(now, data, time_frame_info, min_max_prices):
    ...
    range_msg = f"(entre las {hours[0]} y las {hours[1]})" if frame_type != "valle" else "(entre las 00:00 y las 8:00)" if now.weekday() <= 4 else "(todo el día)"
